...
class EvalSkills:
    ...
    def eval(self):
        ...
        for skill_num in range(self.cfg.skill_dim):
            ...
            while eval_until_episode(episode):
                ...
                while not time_step.last():
                    tau.append(time_step.observation)
                    # ours log
                    if step >= self.cfg.update_skill_every_step and self.cfg.agent.name == "ours":
                        tau = torch.as_tensor(np.concatenate(tau), device=self.cfg.device)
                        with torch.no_grad():
                            q = self.agent.ours(tau)
                            q = F.softmax(q).cpu().numpy()
                            qs.append(np.round(q, 3))
                        step = 0
                        tau = []
                    # diayn log every time
                    if self.cfg.agent.name == "diayn":
                        with torch.no_grad():
                            obs_ = torch.as_tensor([time_step.observation], device=self.cfg.device)
                            pre = self.agent.diayn(obs_)
                            pre = F.softmax(pre).cpu().numpy()
                            loss, acc = self.agent.compute_diayn_loss(obs_, torch.as_tensor([skill], device=self.cfg.device))
                            qs.append([pre, loss.cpu().numpy(), acc.cpu().numpy()])
                        step = 0
...
@hydra.main(config_path='.', config_name='eval_skills')
def main(cfg):
    from eval_skills import EvalSkills as ek
    root_dir = Path.cwd()
    workspace = ek(cfg)
    logging.basicConfig(filename="eval_q.log", encoding="utf-8", level=logging.DEBUG)
