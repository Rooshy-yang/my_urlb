...
class OURDAgent(DDPGAgent):
    ...
    def update_gb(self, skill, next_obs, step):
        metrics = dict()
        labels = torch.argmax(skill, dim=1)
        loss, df_accuracy = self.compute_gb_loss(next_obs, labels)
    ...
    def compute_gb_loss(self, next_obs, skill):
        """
        DF Loss
        """
